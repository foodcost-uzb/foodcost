  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        setBgColor(product.bg_color);
        setBorderColor(product.border_color);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || "Ошибка обновления продукта");
      }
  ...
  if (error && !name) {
    return (
      <div className="max-w-3xl">
        ...
        <Link
          href="/admin/content/products"
          className="inline-block mt-4 text-sm text-[#5838a8] hover:underline"
        >
          Вернуться к списку
        </Link>
      </div>
